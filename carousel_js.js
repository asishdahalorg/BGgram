...
var jsonData = [{"src":"images/home1.jpg","alt":"photo1"},
...
$(function(){
	...
	{
		var newSlide = photos.replace("#src",jsonData[i].src).replace("#alt",jsonData[i].alt);
		$(".slideShow").append(newSlide);
		var newThumbnails = thumbPhotos.replace("#src",jsonData[i].src).replace("#alt",jsonData[i].alt).replace("#data-photo#",i);
